"""
Dialog for editing an existing filament.
"""
...
from utils.db_handler import get_filament_by_id, update_filament, get_all_brands, get_spool_weight
...
class EditFilamentDialog(QDialog):
    ...
    def __init__(self, filament_id: str, parent=None):
        ...
        self.filament = get_filament_by_id(filament_id)
        if not self.filament:
            QMessageBox.critical(self, t("error"), t("filament_not_found"))
            self.reject()
            return

        self.selected_color = QColor(self.filament['color'])
        self.init_ui()

    def init_ui(self):
        """Initialize the dialog UI."""
        self.setWindowTitle(t("edit_filament_title"))
        self.setModal(True)
        ...
        current_brand = self.filament['brand']
        ...
        current_type = self.filament.get('type', 'PLA')
        type_index = self.type_combo.findText(current_type.upper())
        if type_index >= 0:
        ...
        spool_weight = self.filament.get('spool_weight', 0)
        self.without_spool_checkbox = QCheckBox(t("without_spool"))
        self.without_spool_checkbox.setChecked(spool_weight == 0)
        ...
        if spool_weight == 0:
            total_weight = self.filament['initial_weight']
        else:
            total_weight = self.filament['initial_weight'] + spool_weight
        self.weight_input.setValue(total_weight)
        ...
        self.current_weight_label = QLabel(
            t("current_net_weight").format(weight=self.filament['current_weight'])
        )
        layout.addWidget(self.current_weight_label)

        # Initialize display
        self.update_weight_label()
        self.on_brand_changed(self.filament['brand'])
    ...
    def accept_dialog(self):
        ...
        try:
            ...
            QMessageBox.critical(self, t("error"), t("update_filament_error").format(error=str(e)))
